    ...
    while true; do
        ...
        case $U_A in
            ...
            2)
                ...
                jq --arg id "$NEW_UUID" --arg email "$U_NAME" '(.inbounds[]? | select(.tag=="client-in" or .tag=="client-reality-in") | .settings.clients) += [{"id": $id, "flow": "xtls-rprx-vision", "email": $email}]' /usr/local/etc/xray/config.json > /tmp/xray_tmp.json
                ...
                if xray run -test -config /tmp/xray_tmp.json >/dev/null 2>&1; then
                    ...
                    L_NEW="vless://$NEW_UUID@$DOMAIN:443?security=tls&encryption=none&type=tcp&flow=xtls-rprx-vision&fp=chrome&sni=$DOMAIN#Основной"
                    
                    R_PRV=$(jq -r '.inbounds[]? | select(.tag=="client-reality-in") | .streamSettings.realitySettings.privateKey' /usr/local/etc/xray/config.json 2>/dev/null)
                    R_PUB=$(/usr/local/bin/xray x25519 -i "$R_PRV" 2>/dev/null | grep -i "Public" | awk -F':' '{print $2}' | tr -d ' \n\r')
                    R_SID=$(jq -r '.inbounds[]? | select(.tag=="client-reality-in") | .streamSettings.realitySettings.shortIds[0]' /usr/local/etc/xray/config.json 2>/dev/null)
                    R_SNI=$(jq -r '.inbounds[]? | select(.tag=="client-reality-in") | .streamSettings.realitySettings.serverNames[0]' /usr/local/etc/xray/config.json 2>/dev/null)
                    
                    mkdir -p /var/www/html/sub
                    if [ -n "$R_PUB" ] && [ "$R_PUB" != "null" ]; then
                        L_REAL="vless://$NEW_UUID@$DOMAIN:8443?security=reality&encryption=none&pbk=$R_PUB&headerType=none&fp=chrome&type=tcp&flow=xtls-rprx-vision&sni=$R_SNI&sid=$R_SID#Обход БС"
                        echo -e "$L_NEW\n$L_REAL" | base64 -w 0 > "/var/www/html/sub/$NEW_UUID"
                    else
            ...
            3)
                ...
                jq --arg del "$U_DEL" '(.inbounds[]? | select(.tag=="client-in" or .tag=="client-reality-in") | .settings.clients) |= map(select(.id != $del and .email != $del))' /usr/local/etc/xray/config.json > /tmp/xray_tmp.json
                if xray run -test -config /tmp/xray_tmp.json >/dev/null 2>&1; then
                    mv /tmp/xray_tmp.json /usr/local/etc/xray/config.json; systemctl restart xray
            ...
            4)
                ...
                if [ -n "$T_UUID" ]; then
                    local L_BASE="vless://$T_UUID@$DOMAIN:443?security=tls&encryption=none&type=tcp&flow=xtls-rprx-vision&fp=chrome&sni=$DOMAIN#Основной"
                    ...
                    mkdir -p /var/www/html/sub
                    if [ -n "$R_PUB" ] && [ "$R_PUB" != "null" ]; then
                        local L_REAL="vless://$T_UUID@$DOMAIN:8443?security=reality&encryption=none&pbk=$R_PUB&headerType=none&fp=chrome&type=tcp&flow=xtls-rprx-vision&sni=$R_SNI&sid=$R_SID#Обход_БС"
                        echo -e "$L_BASE\n$L_REAL" | base64 -w 0 > /var/www/html/sub/$T_UUID
                    else
                        echo -n "$L_BASE" | base64 -w 0 > /var/www/html/sub/$T_UUID
                    fi
		...
		if isAdmin && adminState != "" {
			...
			if adminState == "invite" {
				...
				script := fmt.Sprintf(`
					...
					jq --arg id "$NEW_UUID" --arg email "$NAME" '(.inbounds[]? | select(.tag=="client-in" or .tag=="client-reality-in") | .settings.clients) += [{"id": $id, "flow": "xtls-rprx-vision", "email": $email}]' /usr/local/etc/xray/config.json > /tmp/xb.json
					...
					L_BASE="vless://$NEW_UUID@$DOM:443?security=tls&encryption=none&type=tcp&flow=xtls-rprx-vision&fp=chrome&sni=$DOM#Основной"
					R_PRV=$(jq -r '.inbounds[]? | select(.tag=="client-reality-in") | .streamSettings.realitySettings.privateKey' /usr/local/etc/xray/config.json 2>/dev/null)
					R_PUB=$(/usr/local/bin/xray x25519 -i "$R_PRV" 2>/dev/null | grep -i "Public" | awk -F':' '{print $2}' | tr -d ' \n\r')
					R_SID=$(jq -r '.inbounds[]? | select(.tag=="client-reality-in") | .streamSettings.realitySettings.shortIds[0]' /usr/local/etc/xray/config.json 2>/dev/null)
					R_SNI=$(jq -r '.inbounds[]? | select(.tag=="client-reality-in") | .streamSettings.realitySettings.serverNames[0]' /usr/local/etc/xray/config.json 2>/dev/null)
					...
					mkdir -p /var/www/html/sub
					if [ -n "$R_PUB" ] && [ "$R_PUB" != "null" ]; then
					    L_REAL="vless://$NEW_UUID@$DOM:8443?security=reality&encryption=none&pbk=$R_PUB&headerType=none&fp=chrome&type=tcp&flow=xtls-rprx-vision&sni=$R_SNI&sid=$R_SID#Обход_БС"
					    echo -e "$L_BASE\n$L_REAL" | base64 -w 0 > /var/www/html/sub/$NEW_UUID
					else
					    echo -n "$L_BASE" | base64 -w 0 > /var/www/html/sub/$NEW_UUID
					fi
				...
				script := fmt.Sprintf(`
					...
					jq --arg del "%s" '(.inbounds[]? | select(.tag=="client-in" or .tag=="client-reality-in") | .settings.clients) |= map(select(.id != $del and .email != $del))' /usr/local/etc/xray/config.json > /tmp/xb.json
...
EOF
        ...
        jq -c '.inbounds[]? | select(.tag=="client-in") | .settings.clients[]' /usr/local/etc/xray/config.json | while read -r client; do
            ...
            L_BASE="vless://$UUID@$NEW_DOMAIN:443?security=tls&encryption=none&type=tcp&flow=xtls-rprx-vision&fp=chrome&sni=$NEW_DOMAIN#Основной"
            ...
            if [ -n "$R_PUB" ] && [ "$R_PUB" != "null" ]; then
                L_REAL="vless://$UUID@$NEW_DOMAIN:8443?security=reality&encryption=none&pbk=$R_PUB&headerType=none&fp=chrome&type=tcp&flow=xtls-rprx-vision&sni=$R_SNI&sid=$R_SID#Обход_БС"
                echo -e "$L_BASE\n$L_REAL" | base64 -w 0 > /var/www/html/sub/$UUID
            else
                echo -n "$L_BASE" | base64 -w 0 > /var/www/html/sub/$UUID
            fi
...
EOF
    ...
    ufw allow 22/tcp; ufw allow 80/tcp; ufw allow 443/tcp; ufw allow 8443/tcp; ufw --force enable >/dev/null 2>&1
...
EOF
    ...
    # Генерация ключей для Reality на RU мосте
    RU_K=$(/usr/local/bin/xray x25519)
    RU_PRV=$(echo "$RU_K" | grep -iE "PrivateKey" | awk -F':' '{print $2}' | tr -d ' \n\r')
    RU_PUB=$(echo "$RU_K" | grep -iE "Password" | awk -F':' '{print $2}' | tr -d ' \n\r')
    RU_SID=$(openssl rand -hex 4 | tr -d ' \n\r')
    echo "---------------------------------------------------------"
    read -p "Введи SNI для обхода белых списков на RU (например, vk.com): " RU_SNI
    RU_SNI=${RU_SNI:-vk.com}; RU_SNI=$(echo "$RU_SNI" | tr -d ' \r\n')
    ...
    cat <<EOF > /tmp/ru_base.json
{
  ...
  "inbounds": [
    ...
    { "tag": "client-reality-in", "port": 8443, "protocol": "vless", "settings": { "clients": [ { "id": "$CLIENT_UUID", "flow": "xtls-rprx-vision", "email": "Admin" } ], "decryption": "none" }, "streamSettings": { "network": "tcp", "security": "reality", "realitySettings": { "show": false, "dest": "$RU_SNI:443", "serverNames": ["$RU_SNI"], "privateKey": "$RU_PRV", "shortIds": ["$RU_SID"] } } }
  ],
  ...
  "routing": { "balancers": [ { "tag": "euro-balancer", "selector": [], "strategy": { "type": "leastPing" } } ], "rules": [ { "type": "field", "inboundTag": ["api-in"], "outboundTag": "api" }, { "type": "field", "inboundTag": ["client-in", "client-reality-in"], "balancerTag": "euro-balancer" }, { "type": "field", "ip": ["geoip:private"], "outboundTag": "block" }, { "type": "field", "protocol": ["bittorrent"], "outboundTag": "block" } ] }
}
EOF
...
EOF
    ...
    L_REAL="vless://$CLIENT_UUID@$DOMAIN:8443?security=reality&encryption=none&pbk=$RU_PUB&headerType=none&fp=chrome&type=tcp&flow=xtls-rprx-vision&sni=$RU_SNI&sid=$RU_SID#Admin_Обход_БС"
    echo -e "$L_RU\n$L_REAL" | base64 -w 0 > "/var/www/html/sub/$CLIENT_UUID"
...
change_ru_sni() {
    ...
    local OLD_SNI=$(jq -r '.inbounds[]? | select(.tag=="client-reality-in") | .streamSettings.realitySettings.serverNames[0]' /usr/local/etc/xray/config.json 2>/dev/null)
    
    if [ -z "$OLD_SNI" ] || [ "$OLD_SNI" == "null" ]; then
        echo "❌ Узел обхода БС (Reality) на RU-сервере еще не создан!"
        return
    fi
    
    echo "Текущий сайт маскировки (SNI) для обхода БС: $OLD_SNI"
    read -p "Введи НОВЫЙ SNI (например, vk.com или yandex.ru): " NEW_SNI
    if [ -z "$NEW_SNI" ]; then echo "Отмена."; return; fi
    
    echo "⏳ Обновляю конфиг..."
    jq --arg sni "$NEW_SNI" '
        .inbounds |= map(
            if .tag == "client-reality-in" then
    ...
    if /usr/local/bin/xray run -test -config /tmp/xray_ru_sni.json >/dev/null 2>&1; then
        ...
        # Обновляем ссылки всем пользователям с новым SNI
        echo "⏳ Обновляю ссылки подписок..."
        bash -c 'CONF="/usr/local/etc/xray/config.json"; DOMAIN=$(ls /etc/letsencrypt/live/ 2>/dev/null | grep -v "^README$" | head -n 1); [ -z "$DOMAIN" ] && DOMAIN=$(curl -s4 ifconfig.me); R_PRV=$(jq -r ".inbounds[]? | select(.tag==\"client-reality-in\") | .streamSettings.realitySettings.privateKey" "$CONF" 2>/dev/null); R_PUB=$(/usr/local/bin/xray x25519 -i "$R_PRV" 2>/dev/null | grep -i "Public" | awk -F":" "{print \$2}" | tr -d " \n\r"); R_SID=$(jq -r ".inbounds[]? | select(.tag==\"client-reality-in\") | .streamSettings.realitySettings.shortIds[0]" "$CONF" 2>/dev/null); R_SNI=$(jq -r ".inbounds[]? | select(.tag==\"client-reality-in\") | .streamSettings.realitySettings.serverNames[0]" "$CONF" 2>/dev/null); jq -c ".inbounds[]? | select(.tag==\"client-in\") | .settings.clients[]" "$CONF" | while read -r client; do UUID=$(echo "$client" | jq -r ".id"); L_BASE="vless://$UUID@$DOMAIN:443?security=tls&encryption=none&type=tcp&flow=xtls-rprx-vision&fp=chrome&sni=$DOMAIN#Основной"; if [ -n "$R_PUB" ] && [ "$R_PUB" != "null" ]; then L_REAL="vless://$UUID@$DOMAIN:8443?security=reality&encryption=none&pbk=$R_PUB&headerType=none&fp=chrome&type=tcp&flow=xtls-rprx-vision&sni=$R_SNI&sid=$R_SID#Обход_БС"; echo -e "$L_BASE\n$L_REAL" | base64 -w 0 > "/var/www/html/sub/$UUID"; fi; done'
        
        echo "✅ SNI для обхода БС успешно изменен на $NEW_SNI!"
    else
        echo "❌ Ошибка конфигурации. Отмена."
...
manage_infrastructure() {
    while true; do
        ...
        echo "6) 🎭 Смена SNI (EU - сайт маскировки)"
        echo "6.1) 🎭 Смена SNI (RU - обход БС)"
        ...
        case $INFRA_ACT in
            ...
            6.1) change_ru_sni; read -n 1 -s -r -p "Нажми любую клавишу..." ;;
            7) speedtest_bridge ;; 
...
full_cleanup() {
    ...
    systemctl stop xray nginx vpn-tg-bot 2>/dev/null
    systemctl disable xray vpn-tg-bot 2>/dev/null
    
    if command -v docker &> /dev/null; then docker rm -f ssantifilter 2>/dev/null; fi
    ...
    rm -f /usr/local/bin/vpn-bot /etc/systemd/system/vpn-tg-bot.service
    rm -rf /usr/src/vpn-bot
	
    systemctl daemon-reload 
    systemctl restart nginx 2>/dev/null
    
    echo "✅ Система полностью очищена. Можно накатывать мост заново."
    read -p "Нажми Enter..."
}
...
while true; do
    clear
    echo "#########################################################"
    echo "🚀 KVN BRIDGE MASTER | Admin Panel v3.11"
    echo "#########################################################"
    show_system_status
